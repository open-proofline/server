ALTER TABLE incidents
  ADD COLUMN IF NOT EXISTS incident_mode TEXT CHECK (
    incident_mode IS NULL
    OR incident_mode IN ('emergency', 'interaction_record', 'safety_check', 'evidence_note')
  ),
  ADD COLUMN IF NOT EXISTS capture_profile TEXT CHECK (
  ...
  ADD COLUMN IF NOT EXISTS escalation_policy TEXT CHECK (
  ...
  ADD COLUMN IF NOT EXISTS sharing_state TEXT CHECK (
