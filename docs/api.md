...
`expires_at` is optional. When omitted, the API applies the configured default emergency-token lifetime, which is 24 hours unless `SAFE_DEFAULT_EMERGENCY_TOKEN_TTL` is changed. Explicit `expires_at` values are preserved. Setting `SAFE_DEFAULT_EMERGENCY_TOKEN_TTL=0` disables the default and lets omitted expiries remain valid until revoked.
